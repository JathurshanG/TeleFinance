...
histo = pd.read_csv('https://github.com/JathurshanG/TeleFinance/blob/master/data/raw/History.csv?raw=true').dropna(subset='price')
histo["Date"] = pd.to_datetime(histo['Date'])

basicInfo = pd.read_csv('https://github.com/JathurshanG/TeleFinance/blob/master/data/raw/informations.csv?raw=true')
basicInfo['compte'] = 1
...
st.sidebar.success("Daily Recap 📊")
...
with col_bar:
    ...
    invest_sector = histo.merge(basicInfo[['ticker', 'shortName']], left_on="ticket", right_on='ticker', how='left')
    invest_sector = invest_sector[invest_sector['Date'] == invest_sector['Date'].max()]
    agg_sector = invest_sector.groupby('shortName', as_index=False).agg({'price': 'sum'}).sort_values(by='price', ascending=False).head(10)
    fig3 = px.bar(agg_sector, x='shortName', y='price', title="Montant investi par secteur", text_auto='.2s')
